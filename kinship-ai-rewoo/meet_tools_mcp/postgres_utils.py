...
def decrypt_credentials(encrypted_data: str) -> Dict[str, Any]:
    ...
    if not encrypted_data:
        logger.warning("[decrypt_credentials] No encrypted data provided")
        return {}
    
    try:
        key = _get_encryption_key()
        logger.debug(f"[decrypt_credentials] Using encryption key (first 10 chars): {key[:10].decode()}...")
        fernet = Fernet(key)
        decrypted = fernet.decrypt(encrypted_data.encode()).decode()
        creds = json.loads(decrypted)
        logger.info(f"[decrypt_credentials] Successfully decrypted credentials. Keys found: {list(creds.keys())}")
        return creds
    except Exception as e:
        logger.error(f"[decrypt_credentials] Failed to decrypt: type={type(e).__name__}, message={str(e)}")
        logger.error(f"[decrypt_credentials] Encrypted data length: {len(encrypted_data)} chars")
        logger.error(f"[decrypt_credentials] ENCRYPTION_SECRET_KEY length: {len(ENCRYPTION_SECRET_KEY)} chars")
        return {}
...
def get_google_credentials_from_postgres(worker_id: str) -> Dict[str, Any]:
    ...
    """
    logger.info(f"[get_google_credentials_from_postgres] Fetching credentials for worker_id={worker_id}")
    
    # Get tool connection
    connection = get_tool_connection_by_worker_id(worker_id)
    
    if not connection:
        logger.warning(f"[get_google_credentials_from_postgres] No tool connection found for worker_id={worker_id}")
        return {
            "success": False,
    ...
    logger.info(f"[get_google_credentials_from_postgres] Found tool connection: tool_names={connection.get('tool_names')}, status={connection.get('status')}")
    
    # Check if Google is connected
    tool_names = connection.get("tool_names", [])
    if "google" not in tool_names:
        logger.warning(f"[get_google_credentials_from_postgres] 'google' not in tool_names: {tool_names}")
        return {
            "success": False,
    ...
    # Decrypt credentials
    encrypted_data = connection.get("credentials_encrypted", "")
    logger.info(f"[get_google_credentials_from_postgres] Attempting to decrypt credentials (encrypted length={len(encrypted_data) if encrypted_data else 0})")
    
    all_creds = decrypt_credentials(encrypted_data)
    
    if not all_creds:
        logger.error(f"[get_google_credentials_from_postgres] Decryption returned empty dict - check ENCRYPTION_SECRET_KEY")
        return {
            "success": False,
            "message": "Failed to decrypt credentials - check ENCRYPTION_SECRET_KEY matches kinship-agent",
            "creds": None,
        }
    
    if "google" not in all_creds:
        logger.error(f"[get_google_credentials_from_postgres] 'google' key not in decrypted credentials. Keys found: {list(all_creds.keys())}")
        return {
            "success": False,
            "message": f"Google credentials not found in tool connection. Available providers: {list(all_creds.keys())}",
            "creds": None,
        }
