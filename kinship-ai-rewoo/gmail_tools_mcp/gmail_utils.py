"""
Common utility functions shared across MCP tools.

Includes:
- Fetching wallet address by username
- Checking active membership status
- Unified credential fetching (PostgreSQL + MongoDB)
...
try:
    ...
    print(MONGO_URI, "=== MONGO_URI ===", MONGO_DB_NAME)
    ...
    mongo_client = None
    mongo_db = None
    google_token_collection = None
    users_collection = None
    membership_collection = None
    # mongo_collection = None


def get_wallet_by_username(filter: str) -> Dict[str, Optional[str]]:
    """
    Fetch wallet and email using username OR email OR wallet address
    from mmosh-users collection.
    ...
    """
    logger.info(f"[get_wallet_by_username] Looking up filter='{filter}'")

    if users_collection is None:
        logger.error("[get_wallet_by_username] Users collection not initialized")
        return {
            "success": False,
            "wallet": None,
            "email": None,
            "message": "Users collection not initialized",
        }

    if not filter:
        logger.warning("[get_wallet_by_username] Empty filter provided")
        return {
            "success": False,
            "wallet": None,
            "email": None,
    ...
    logger.info(f"[get_wallet_by_username] MongoDB query: {query}")

    user = users_collection.find_one(
        query,
        {"wallet": 1, "email": 1},
    )
    logger.info(f"[get_wallet_by_username] MongoDB result: {user}")

    if not user:
        logger.warning(f"[get_wallet_by_username] No user found for filter='{filter}'")
        return {
            "success": True,
            "wallet": None,
            "email": None,
    ...
    logger.info(f"[get_wallet_by_username] Found user: wallet={wallet}, email={email}")
...
# def has_membership(filter: str) -> Dict[str, Any]:
#     """
...
#             "wallet": wallet,
#             "email": email,
#             "message": "Membership collection not initialized",
#         }
...
def has_member(filter: str) -> Dict[str, Any]:
    ...
    # Hard failure (DB error, invalid input, etc.)
    if not user_result["success"]:
        return {
            "success": False,
            "has_membership": False,
...
def get_google_credentials_by_wallet(wallet: str) -> dict:
    """
    Fetch Google OAuth credentials using wallet.
    Google SDK REQUIRES naive UTC datetimes.
    """

    if google_token_collection is None:
        return {
            "success": False,
    ...
    if not token_doc:
        return {
            "success": False,
            "message": f"No Google OAuth record found for userId={wallet}",
            "creds": None,
        }
    ...
    if not access_token:
        return {
            "success": False,
            "message": "Access token missing in database",
            "creds": None,
        }
    ...
    if not creds.expiry or creds.expiry <= refresh_threshold:
        ...
        logger.info("Refreshing Google access token for agentId=%s", wallet)

        try:
            creds.refresh(Request())
        except Exception:
            logger.error(
                "Google token refresh failed for userId=%s",
                wallet,
                exc_info=True,
            )
            return {
        ...
        logger.info("Google token refreshed and stored for userId=%s", wallet)

    return {
        "success": True,
        "message": "Google access token ready",
        "creds": creds,
    }
...
def check_send_permission(
    ...
    """
    Check if the user has permission to send email to the receiver.
...
def find_message_ids_by_text(service, query_text: str, max_results: int = 5):
    """
    Search Gmail messages by text (subject/body).
    ...
    return {
        "success": True,
        "matches": messages,  # contains id + threadId
...
def build_raw_message(to: str, subject: str, body: str):
    """
    Build base64 encoded raw email.
    """

    raw = f"To: {to}\r\n" f"Subject: {subject}\r\n\r\n" f"{body}"

    return {"raw": base64.urlsafe_b64encode(raw.encode("utf-8")).decode("utf-8")}


async def schedule_drive_file_deletion(
    drive_service,
    file_id: str,
    ttl_seconds: int,
):
    """
    Deletes a Google Drive file after TTL seconds.
    This is the ONLY supported way to expire public links.
    """
    try:
        await asyncio.sleep(ttl_seconds)
        drive_service.files().delete(fileId=file_id).execute()
        logger.info("Auto-deleted Drive file: %s", file_id)
    except Exception as e:
        logger.error("Failed to auto-delete Drive file %s: %s", file_id, e)
